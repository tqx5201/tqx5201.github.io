...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def categoryContent(self, cid, page, filter, ext):
		...
		if cid == 'bilibili':
			...
			remarknm = 'nick'
			'''
			url = f'https://live.huya.com/liveHttpUI/getLiveList?iGid={tid}&iPageNo={page}&iPageSize=120'
			#url = f'https://live.huya.com/liveHttpUI/getTmpLiveList?iGid={tid}&iPageNo={page}&iPageSize=120'#&iLibId=2213'
			r = self.fetch(url, headers=header, timeout=5)
			data = json.loads(r.text)
			vodList = data['vList']
			pagecount = data['iTotalPage']
			append = 'huya'
			imgnm = 'sScreenshot'
			vidnm = 'lProfileRoom'
