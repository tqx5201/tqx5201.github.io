...
$empty_tmp = 1;
// 1 入库后清除临时表的数据, 0 保留临时表的数据,建议调测期间设置为0, 调测完毕后修改为1,减少存储空间
$deleteoffset = -8;
// 清理xx天前的节目数据
//error_reporting(0);
// 禁止输出错误提示
...
// 节目总表地址列表
$xmlurl[] = 'https://e.erw.cc/e.xml.gz';
//$xmlurl[] = 'https://assets.livednow.com/epg.xml';
//$xmlurl[] = 'https://epg.pw/xmltv/epg_CN.xml.gz';
//$xmlurl[] = 'https://epg.pw/xmltv/epg_HK.xml.gz';
$xmlurl[] = 'https://epg.pw/xmltv/epg_TW.xml.gz';
...
// 当前url
$url = $_SERVER['HTTP_HOST']??'127.0.0.1'. $_SERVER['REQUEST_URI']??'';
// 获取最后来源地址
...
foreach ($xmlurl as $key=>$url){
    ...
    $data = getContent($url);
    //echo $data;
    echo '第【'.($key+1).'】个地址,'. $url.'<br>'.PHP_EOL;
    if (!is_string($data)){
        echo '<font color="red">获取失败！<br>'.PHP_EOL;
        continue;
    }else{
        echo '<font color="green">获取成功！<br>'.PHP_EOL;
    }
    // 解析xml数据
    ...
    if ($n > 0){
        ...
        echo '数据日期:' . $currentDate . ' <- ' . $formattedNewDate . '<br>'.PHP_EOL;
        $channel->exec("delete from `epg_programme` where sdate < '{$formattedNewDate}';");
        echo '从epg_programme表删除了' . $channel->changes() . '条记录。 <br><br>'.PHP_EOL;
        if ($empty_tmp == 1){
            // 清空临时表
            $channel->exec("delete from tmp_epg_channel");
            $channel->exec("delete from tmp_epg_programme");
...
//压缩数据库
$channel->exec('VACUUM;');
echo '压缩数据库成功<br>'.PHP_EOL;
$channel->close();
echo '执行总用时' . $executionTime . ' 秒'.PHP_EOL;

file_put_contents('channel_epg.db.gz',gzencode(file_get_contents('channel_epg.db')));
echo '生成db.gz成功<br>'.PHP_EOL;
